"""
IPL Crowd Safety Management Dashboard — Streamlit + Cohere AI
...
Folder structure:
  ...
  ├── app.py
  ├── .streamlit/
  │   └── secrets.toml          ← COHERE_API_KEY = "your_key_here"
  └── data/
      ├── dim_gate.xlsx
      ├── dim_stadium.xlsx
      ├── dim_zone.xlsx
...
# ─────────────────────────────────────────────────────────
# THEMES — one accent colour per page
# ─────────────────────────────────────────────────────────
THEMES = {
    "Overview": {
...
.dash-title {{
    font-family: 'Sora', sans-serif; font-size: 23px;
    font-weight: 800; color: {t['text']}; margin: 0;
}}
.dash-sub {{ font-size: 12.5px; color: {t['text2']}; margin-top: 5px; }}

.kpi-card {{
    background: {t['card']}; border: 1px solid {t['border']};
    border-radius: 16px; padding: 16px 18px 14px; min-height: 100px;
...
.kpi-label {{
    ...
    letter-spacing: 0.9px; color: {t['text2']}; margin-bottom: 7px;
}}
.kpi-val {{
    font-family: 'Sora', sans-serif; font-size: 27px;
    font-weight: 800; color: {t['text']}; line-height: 1;
}}
.kpi-sub {{ font-size: 10px; color: {t['text2']}; margin-top: 6px; }}

.sec-lbl {{
    font-family: 'Sora', sans-serif; font-size: 12px; font-weight: 700;
    color: {t['accent2']}; text-transform: uppercase; letter-spacing: 0.9px;
    margin: 14px 0 8px 0; padding-bottom: 5px; border-bottom: 2px solid {t['accent_lt']};
}}
...
.ai-metric {{
    font-size: 28px; font-weight: 800; color: {t["text"]}; font-family: 'Sora', sans-serif;
}}
.ai-label {{
    font-size: 11px; text-transform: uppercase; font-weight: 700;
    letter-spacing: 0.7px; color: {t["text2"]};
}}
.ai-status-critical {{ color: #DC2626; font-weight: 700; font-size: 12px; }}
.ai-status-warning  {{ color: #F59E0B; font-weight: 700; font-size: 12px; }}
.ai-status-good     {{ color: #10B981; font-weight: 700; font-size: 12px; }}
...
def generate_cohere_insights(summary_text, temperature_value=0.4, token_value=700):
    """Call Cohere command-r-plus and return insight text."""
    api_key = get_cohere_key()
    if not api_key:
        return (
            "⚠️ Cohere API key not found.  \n"
...
def add_advanced_risk_features(df):
    ...
    df["risk_reason"] = np.select(
        ...
        ["High bottleneck risk", "Delayed medical response",
         "Long queue wait time", "High heat exposure", "High security activity"],
...
<div style="text-align:center;padding:6px 0 14px 0;">
  ...
  <div style="font-size:9.5px;letter-spacing:0.8px;font-weight:600;opacity:0.5;margin-top:2px;">
    ...
    st.markdown(
        ...
        'margin:0 0 8px 0;opacity:0.6;">FILTERS</p>', unsafe_allow_html=True
    )
    ...
    st.markdown(
        '<p style="font-size:10px;font-weight:700;letter-spacing:0.8px;'
        'margin:0 0 8px 0;opacity:0.6;">COHERE AI SETTINGS</p>', unsafe_allow_html=True
    )
...
cap_          = round(f["capacity_breach"].mean() * 100, 2)
...
summary_text = f"""
Stadiums: {', '.join(map(str, sel_stadium))} | Phases: {', '.join(map(str, sel_phase))}
...
Capacity Breach: {cap_breach}% | Resolution Rate: {res_rate}%
Ambulance Response: {amb_resp} min | Queue Wait: {avg_queue} min
Crowd Pressure Index: {avg_pressure} | Bottleneck Risk: {avg_bottleneck}
Heat Risk Index: {avg_heat} | High Risk Zones: {high_risk_zones}
Delayed Medical Zones: {delayed_med} | Unauthorized Entries: {unauthorized}
Counterfeit Cases: {counterfeit} | Pitch Invasions: {pitch_inv}
Fan Ejections: {fan_ej} | Required Staff: {req_staff}
Required Barricades: {req_barr} | Medical Teams: {med_teams}
...
<div class="ai-card">
  ...
  This panel uses filtered KPIs, advanced risk scores, anomaly detection, and the risk
  priority matrix to generate Cohere-powered operational recommendations.
</div>""", unsafe_allow_html=True)
    ...
    if generate_ai:
        with st.spinner("Generating Executive AI Insights via Cohere..."):
            insights = generate_cohere_insights(summary_text, ai_temperature, ai_max_tokens)

        tab1, tab2, tab3, tab4 = st.tabs(
            ["📌 Key Insights", "⚠️ Risks", "✅ Recommendations", "📖 Detailed AI Output"])
...
<ul>
  <li>Overall risk score is <b>{overall_risk_score}</b>, showing current safety pressure level.</li>
...
<ul>
  <li>🔴 High bottleneck risk zones need immediate control support.</li>
  <li>🔴 Delayed ambulance response affects emergency readiness.</li>
  <li>🟠 Long queue wait times increase congestion during entry/exit phase.</li>
...
# ═══════════════════════════════════════════════════════════
    ...
    with c2:
        ...
        st.dataframe(mat.style.format("{:.2f}").background_gradient(cmap="Blues", axis=None),
...
# ═══════════════════════════════════════════════════════════
    ...
    security_risk = risk_matrix[
        risk_matrix["Main Risk Reason"].isin(["High security activity"])
    ].head(10)
...
# ═══════════════════════════════════════════════════════════
    ...
    st.dataframe(mat2.style.format("{:.2f}").background_gradient(cmap="Greens", axis=None),
                 use_container_width=True)
...
<ul>
  ...
  <li><b>Business value:</b> Streamlit is used as an AI-powered operational decision app powered by Cohere AI.</li>
</ul>
</div>""", unsafe_allow_html=True)
...
st.markdown(
    ...
    " Streamlit + Plotly + Cohere AI &nbsp;|&nbsp; Advanced Risk Matrix Enabled"
