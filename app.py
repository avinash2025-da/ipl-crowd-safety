...
.dash-title {{ font-family: 'Sora', sans-serif; font-size: 23px; font-weight: 800; color: {t['text']}; margin: 0; }}
...
.kpi-label {{ font-size: 10px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.9px; color: {t['text2']}; margin-bottom: 7px; }}
.kpi-val {{ font-family: 'Sora', sans-serif; font-size: 27px; font-weight: 800; color: {t['text']}; line-height: 1; }}
...
.ai-metric {{ font-size: 28px; font-weight: 800; color: {t["text"]}; font-family: 'Sora', sans-serif; }}
.ai-label {{ font-size: 11px; text-transform: uppercase; font-weight: 700; letter-spacing: 0.7px; color: {t["text2"]}; }}
...
[data-testid="stDataFrame"] {{ border-radius: 12px; border: 1px solid {t['border']}; overflow: hidden; }}
...
def generate_cohere_insights(summary_text, temperature_value=0.4, token_value=700):
    ...
    if not api_key:
        return "⚠️ Cohere API key not found. Add it in Streamlit Cloud Secrets as: COHERE_API_KEY = \"your_key\""
    try:
...
def ask_ai_question(question, context_text, temperature_value=0.3, token_value=500):
    api_key = get_cohere_key()
    if not api_key:
        return "⚠️ Cohere API key not found."
    try:
        co = cohere.Client(api_key)
        prompt = f"""
...
# ═══════════════════════════════════════════════════════════
    ...
    with c2:
        ...
        # FIX: use background_gradient without cmap to avoid matplotlib dependency
        st.dataframe(
...
# ═══════════════════════════════════════════════════════════
    ...
    # FIX: removed cmap parameter to avoid matplotlib dependency
